...
import { calculateCheckout } from '@/server/services';
...
export async function POST(request: Request) {
  try {
    ...
    return jsonResponse(await calculateCheckout(payload));
  } catch (error) {
    return errorResponse(
      error instanceof Error ? error.message : 'Не удалось рассчитать корзину',
    );
  }
}
