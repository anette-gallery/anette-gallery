...
import { validateGiftCard } from '@/server/services';
...
export async function POST(request: Request) {
  try {
    ...
    return jsonResponse(await validateGiftCard(payload));
  } catch (error) {
    return errorResponse(
      error instanceof Error ? error.message : 'Не удалось проверить сертификат',
    );
  }
}
